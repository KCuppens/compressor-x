        ...
        for index, child in enumerate(children):
            if isinstance(child, tuple):
                ...
                if dissected["translatable"]:
                    ...
                    if query_languages:
                        relation = LOOKUP_SEP.join(dissected["relation"] + ["translations"])
                        field_supp = (
                            (LOOKUP_SEP + dissected["supplement"])
                            if dissected["supplement"]
                            else ""
                        )
                        lang_supp = (
                            (LOOKUP_SEP + "in")
                        ...
                        q = Q(
                            **{
                                ...
                                "{}__language{}".format(relation, lang_supp): query_languages,
                            }
                        )
                else:
                    q = Q(**{child[0]: child[1]})
                ...
                if child.lang:
                    ...
                    q = getter(*child.children, _connector=child.connector, _negated=child.negated)
                else:
                    q = _get_translations_query(
                        *child.children, _connector=child.connector, _negated=child.negated
                    )
