...
from apps.compress.services.AutoCompress import AutoCompress, CustomCompress
    ...
    def compress(action_obj):
        ...
        for compression in compressions:
            ...
            if config_file.custom:
                custom_compress(action_obj, compression, init_file, count, step)
            else:
                compression_file, count = auto_compress(action_obj, compression, count, step)
            ...
            count += step
        # Update status
        send_progress_websocket(action_obj.id, 100, "Compression completed.")
        return "Succesfully completed the compression."

    def check_supported_file_type(file):
...
def custom_compress(action_obj, compression_obj, init_file, count, step):
    ...
    img = PillImage.open(init_file.file)
    # Customize size
    height, width, override = CustomCompress().customize_size(img, action_obj)
    # Setting variables
    config_file = action_obj.config_file
    ...
    img = CustomCompress().resize_image(
        config_file, override, img, height, width, resize_fit, compression_filter
    )
    # Websocket status
    send_progress_websocket(
        action_obj,
